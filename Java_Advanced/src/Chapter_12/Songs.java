...
// A small "data source" class — its only job is to hand back a
// ready-made playlist. Nothing else in the program needs to know HOW
// the songs are built; it just calls getSongs() and gets a List back.
public class Songs {
 
    // Returns an immutable List<Song> — List.of(...) builds a list you
    // can't add to or remove from afterward, which is appropriate here
    // since this is meant to be a fixed, canned dataset rather than
    // something callers should be able to modify.
    public List<Song> getSongs() {
        return List.of(
                // Each Song is built via the constructor you write in
                // Song.java — the order of arguments here (title, artist,
                // genre, year, timesPlayed) needs to match whatever
                // parameter order you choose for that constructor.
